...
class CheckCombinationGameRulesProvider: RggRulesProvider {

    override val ruleIds = listOf(MY_CUSTOM_GAME_ID_LOCAL, MY_CUSTOM_GAME_ID_REST).map {
        RggRulesId(MY_CUSTOM_PROVIDER_ID, it)
    }
    ...
    private val customGames: Map<String, URL> = listOf(MY_CUSTOM_GAME_ID_LOCAL, MY_CUSTOM_GAME_ID_REST).associateWith {
        CheckCombinationGameRulesProvider::class.java.getResource("resources/$it.yaml")!!
    }

    companion object {
        const val MY_CUSTOM_PROVIDER_ID = "RestGameProvider"
        const val MY_CUSTOM_GAME_ID_LOCAL = "SimulationServiceExample"
        const val MY_CUSTOM_GAME_ID_REST = "SimulationServiceExample_Rest"
    }
